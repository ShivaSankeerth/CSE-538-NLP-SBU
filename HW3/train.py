...
def train(model: nn.Module,
          ...
          num_epochs: int,
          batch_size: int) -> Dict[str, Union[nn.Module, str]]:
    """
    ...
    for epoch in range(num_epochs):
        ...
        predicted_trees = predict(model, device, validation_sentences, parsing_system, vocabulary)
...
if __name__ == '__main__':
    ...
    if args.pretrained_embedding_file:
        ...
        model.embeddings = nn.Embedding.from_pretrained(torch.Tensor(embedding_matrix).to(device),freeze=args.trainable_embeddings)
    ...
    torch.save(trained_model.state_dict(), os.path.join(save_serialization_dir, f'model.pt'))
