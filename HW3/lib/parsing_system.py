...
class ParsingSystem:
    ...
    def apply(self, configuration: Configuration, transition: str) -> Configuration:
        ...
        if(transition[0:2]=='L('):
            #Left Reduce
            head_node = configuration.get_stack(0)
            dependent_node = configuration.get_stack(1)
            label = transition[2:-1]
            configuration.add_arc(head_node, dependent_node , label)
            configuration.remove_second_top_stack()
        elif(transition[0:2]=='R('):
            #Right Reduce
            head_node = configuration.get_stack(1)
            dependent_node = configuration.get_stack(0)
            label = transition[2:-1]
            configuration.add_arc(head_node, dependent_node , label)
