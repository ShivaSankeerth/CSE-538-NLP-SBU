...
class ProbingClassifier(nn.Module):
    def __init__(self,
        ...
        # TODO(students): start
        self.probe_layer = nn.Linear(self.input_dim,self.class_num)
        # TODO(students): end
    ...
    def forward(self, inputs: torch.Tensor, training: bool = False) -> torch.Tensor:
        ...
        layer_representations,logits =  outputs['layer_representations'],outputs['logits']
        nth_layer_rep = layer_representations[:, self._layer_num - 1, :]
        logits = self.probe_layer(nth_layer_rep)
