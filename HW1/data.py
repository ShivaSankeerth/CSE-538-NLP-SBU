...
class Dataset:
    ...
    def generate_batch(self):
        ...
        while(self.batch_size > words_in_batch):
            start, end = words_in_batch , words_in_batch+self.num_skips 
            
            word_window = self.data[self.data_index-self.skip_window:self.data_index] + self.data[self.data_index+1:self.data_index+1+self.skip_window]

            context_word[start:end] = self.data[self.data_index]

            center_word[start:end] = np.random.choice(word_window, size=self.num_skips, replace=False)

            words_in_batch, self.data_index = self.num_skips + words_in_batch, stride + self.data_index
